...
psam <- function(obj_sp1, obj_sp2, correction = 'none') {
  ...
  switch (correction,
          ...
          'guard' = {
            ...
            min_row <- apply(sp_ID_dist(obj_sp1_ng, obj_sp2), 1, min)
            min_col <- apply(sp_ID_dist(obj_sp1, obj_sp2_ng), 2, min)
          }
  )
  ...
  switch (correction,
          ...
          'torus' = {
            ...
            for(i in seq_along(r)) {
              obj_sp1_t <- torus_corr(obj_sp1)
              obj_sp2_t <- torus_corr(obj_sp2)
              aux <- rgeos::gBuffer(obj_sp1_t, width = r[i])
              aux <- rgeos::gIntersection(aux, obj_sp2)
              areas_1 <- ifelse(is.null(aux), 0, rgeos::gArea(aux))
              rm(aux)

              aux <- rgeos::gBuffer(obj_sp2_t, width = r[i])
              aux <- rgeos::gIntersection(aux, obj_sp1)
              areas_2 <- ifelse(is.null(aux), 0, rgeos::gArea(aux))
              rm(aux)
          ...
          'guard' = {
            ...
            for(i in seq_along(r)) {
              aux <- rgeos::gBuffer(obj_sp1_ng, width = r[i])
              aux <- rgeos::gIntersection(aux, obj_sp2)
              areas_1 <- ifelse(is.null(aux), 0, rgeos::gArea(aux))
              rm(aux)

              aux <- rgeos::gBuffer(obj_sp2_ng, width = r[i])
              aux <- rgeos::gIntersection(aux, obj_sp1)
              areas_2 <- ifelse(is.null(aux), 0, rgeos::gArea(aux))
              rm(aux)
          ...
          'adjust' = {
            ...
            for(i in seq_along(r)) {
              ...
              k21 <- (tot_2/prop_inside_1)*areas_1
              output$pk12[i] <- ((tot_1*tot_2)^(-1))*N*(k21 + k12)/(tot_1 + tot_2)
            }
          })
  ...
  switch (correction,
          ...
          'torus' = {
            ...
            for(i in seq_along(r)) {
              ...
              k12 <- sum(mat_dist1 < r[i])
              k21 <- sum(mat_dist2 < r[i])
              output$pk12[i] <- ((tot_1*tot_2)^(-1))*N*(tot_2*k12 + tot_1*k21)/(tot_1 + tot_2)
            }
          },
          'guard' = {
            new_bbox <- make_guard(obj_sp1@bbox)
          ...
          'adjust' = {
            ...
            for(i in seq_along(r)) {
              ...
              obj_sp1_bf <- rgeos::gArea(rgeos::gIntersection(obj_sp1_bf, limits_to_sp(obj_sp1@bbox),byid = T), byid = T)/rgeos::gArea(obj_sp1_bf, byid = T)
              obj_sp2_bf <- rgeos::gArea(rgeos::gIntersection(obj_sp2_bf, limits_to_sp(obj_sp2@bbox),byid = T), byid = T)/rgeos::gArea(obj_sp2_bf, byid = T)
              mat_dist <- sp_ID_dist(obj_sp1, obj_sp2)
              w <- (tot_1*obj_sp1_bf + tot_2*obj_sp2_bf)/(tot_1 + tot_2)
              output$pk12[i] <- ((tot_1*tot_2)^(-1))*N*(sum((mat_dist < r[i])/w[col(mat_dist)]))
            }
          }
  )
